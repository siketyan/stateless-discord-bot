...
impl Context {
    fn env(&self, key: &str) -> Result<&String, Error> {
        self.env
            .get(key)
            .ok_or(Error::EnvironmentVariableNotFound(key.to_string()))
    }
    ...
    fn perform_verification(&self) -> Result<(), Error> {
        ...
        verify_signature(public_key, signature, timestamp, &self.request.body)
            .map_err(|error| Error::VerificationFailed(error))
    }
    ...
    fn handle_payload(&self) -> Result<String, Error> {
        ...
        let interaction = serde_json::from_str::<Interaction>(payload)
            .map_err(|error| Error::JsonFailed(error))?;
        let response = interaction.perform()?;

        serde_json::to_string(&response).map_err(|error| Error::JsonFailed(error))
    }
    ...
    pub(crate) fn handle_http_request(&self) -> HttpResponse {
        let result = self
            ...
            .map_err(|error| HttpError::from(error));
